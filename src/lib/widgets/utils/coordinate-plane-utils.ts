...
export function setupCoordinatePlaneBaseV2(
	...
	if (data.title) {
		const measured = estimateWrappedTextDimensions(
			data.title,
			chartArea.width,
			CHART_TITLE_FONT_PX
		)
	...
	const maxYTickWidth = Math.max(
		0,
		...yTicks.labels.map((l) => l.length * LABEL_AVG_CHAR_WIDTH_PX)
	)
	...
	const yRes = computeAndRenderYAxis(
		yAxisLegacySpec,
		chartArea,
		canvas,
		yAxisLabelX
