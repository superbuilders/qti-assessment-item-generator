 ...
 */
const FractionGroupSchema = z.object({
    numerator: z.number().int().positive().describe("The number of segments in this group, representing the numerator of the fraction."),
    color: z.string().regex(CSS_COLOR_PATTERN, "invalid css color").describe("The fill color for this group's segments.")
}).strict();

/**
 * Main Zod schema for the fractionSumDiagram widget.
 */
export const FractionSumDiagramPropsSchema = z.object({
    ...
    denominator: z.number().int().positive().describe("The common denominator, which also defines the total number of segments in the tape."),
    groups: z.array(FractionGroupSchema).min(1).describe("An array of colored segment groups, each representing an addend in the sum. The sum of the numerators must not exceed the denominator.")
}).strict()
.describe("Creates a tape diagram to visually represent the sum of fractions with a common denominator. It shows colored segments for each addend, with mathematical expressions for the sum above and below the tape.");
...
export const generateFractionSumDiagram: WidgetGenerator<typeof FractionSumDiagramPropsSchema> = async (props) => {
    ...
    if (totalNumerator > denominator) {
        ...
        throw errors.wrap(ErrInvalidFractionSum, `Sum of numerators (${totalNumerator}) cannot be greater than the denominator (${denominator}).`);
    }
    ...
    let topBlock = estimatedExprBlock(topExprFontStart) + archLabelClearance + bracketHeight + gapBetweenExprAndBracket + gapBetweenBracketAndTape;
    let bottomBlock = totalNumerator > 0 ? (estimatedExprBlock(bottomExprFontStart) + archLabelClearance + bracketHeight + gapBetweenExprAndBracket + gapBetweenBracketAndTape) : 0;
    // Ensure cells are taller than they are wide by targeting a height ratio vs segment width
    const desiredMinHeight = Math.ceil(segmentWidth * 1.30);
    ...
    if (minRequired > height) {
        ...
        if (blocksOnly > availableForBlocks && blocksOnly > 0) {
            ...
            topBlock = estimatedExprBlock(topExprFontStart) + archLabelClearance + bracketHeight + gapBetweenExprAndBracket + gapBetweenBracketAndTape;
            bottomBlock = totalNumerator > 0 ? (estimatedExprBlock(bottomExprFontStart) + archLabelClearance + bracketHeight + gapBetweenExprAndBracket + gapBetweenBracketAndTape) : 0;
        }
    ...
    const tapeY = Math.floor(PADDING + estimatedExprBlock(topExprFontStart) + bracketHeight + gapBetweenExprAndBracket + gapBetweenBracketAndTape);
    ...
    const fitFractionFont = (num: string, den: string, maxWidth: number, maxHeight: number, start: number): number => {
        ...
        for (;;) {
            const numMetrics = estimateWrappedTextDimensions(num, Infinity, fontPx, 1.1);
            const denMetrics = estimateWrappedTextDimensions(den, Infinity, fontPx, 1.1);
    ...
    const drawFraction = (num: string, den: string, cx: number, y: number, fontPx: number) => {
        ...
        canvas.drawText({ x: cx, y: y + (6 + fontPx * 0.60), text: den, fontPx, anchor: "middle" });
    };

    const measureFractionBarWidth = (num: string, den: string, fontPx: number): number => {
        const numMetrics = estimateWrappedTextDimensions(num, Infinity, fontPx, 1.2);
        const denMetrics = estimateWrappedTextDimensions(den, Infinity, fontPx, 1.2);
        return Math.max(numMetrics.maxWidth, denMetrics.maxWidth) + 6;
    };
    ...
    for (const group of groups) {
        for (let i = 0; i < group.numerator; i++) {
            ...
            const cellFont = fitFractionFont("1", String(denominator), Math.floor(segmentWidth - 4), Math.floor(tapeHeight - 6), 56);
    ...
    groups.forEach((group) => {
        ...
        const endX = runningX + groupWidth;
        const cx = Math.floor((startX + endX) / 2);
        const fontPx = fitFractionFont(String(group.numerator), String(denominator), Math.floor(groupWidth - 10), 9999, topExprFontStart);
        const barWidth = measureFractionBarWidth(String(group.numerator), String(denominator), fontPx);
        const y = topExpressionY - archLabelClearance;
        topFractions.push({ startX, endX, cx, fontPx, barWidth, y });
        runningX += groupWidth;
    });
    ...
    if (totalNumerator > 0) {
        ...
        const bottomFont = fitFractionFont(String(totalNumerator), String(denominator), Math.floor(coloredWidth - 10), 9999, bottomExprFontStart);
