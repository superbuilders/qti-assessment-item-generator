...
export function createWidgetSelectionPromptSection(collection: WidgetCollection): string {
	...
	for (const key of allowedWidgetTypes) {
		...
		if (!allWidgetSchemas[key as keyof typeof allWidgetSchemas]) {
	...
	for (const key of allowedWidgetTypes) {
		...
		if (schema) {
			widgetSchemas[key] = z.toJSONSchema(schema)
		}
	}
...
You MUST choose a \`widgetType\` from this exact list: \`[${Array.from(allowedWidgetTypes).map((t) => `"${t}"`).join(", ")}]\`.
...
export function formatUnifiedContextSections(
	...
	if (imageContext.imageUrls && imageContext.imageUrls.length > 0) {
		sections.push("\n## Visual Context:")
		sections.push(`This assessment includes ${imageContext.imageUrls.length} raster image(s) provided as multimodal input for your vision capabilities.`)
