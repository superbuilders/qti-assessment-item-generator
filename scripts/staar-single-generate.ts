...
if (!OUTPUT_DIR_ARG) {
	...
	throw errors.new("Output directory must be provided as second argument")
}

const ROOT = path.resolve(process.cwd(), ROOT_DIR)
const QUESTION_DIR = path.join(ROOT, QUESTION_DIR_ARG)
...
async function processQuestionDir(dir: string, openai: OpenAI): Promise<void> {
	...
	if (htmlFilenameResult.error) {
		...
		throw htmlFilenameResult.error
	}
...
async function main() {
	...
	logger.info("starting single question generation", {
		questionDir: QUESTION_DIR_ARG,
		collection: WIDGET_COLLECTION.name
	})

	await processQuestionDir(QUESTION_DIR, openai)

	logger.info("completed single question generation successfully")
}
