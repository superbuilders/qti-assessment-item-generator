...
const decimalWords = (value: number, places: number, useHyphen: boolean): string => {
	const words = value < 20 ? toWords0to19(value) : twoDigitWords(value, useHyphen)
...
const decimalML = (digitsAfterDecimal: string): string => `<mn>0.${digitsAfterDecimal}</mn>`

export default function generateTemplate(seed: bigint): AssessmentItemInput<TemplateWidgets> {
	...
	const decimalDigits = places === 1 ? `${decimalValue}` : decimalValue.toString().padStart(2, "0")
	...
	const compressedWholeWords = wholeNumberToWords(compressedWholeValue, useHyphen)
	...
	const question = questionVariants[random.nextInt(0, questionVariants.length - 1)]
	...
	const buildCorrectFeedback = (): FeedbackContent<TemplateWidgets> => ({
		preamble: {
			...
			summary: [
				{
					type: "text",
					content: "Your wording matches each expanded part and the decimal place value."
				}
			]
		},
		steps: [
			{
				...
				content: [
					{
						type: "paragraph",
						content: [{ type: "text", content: "Expanded form: " }, { type: "math", mathml: expandedMathML }]
					},
					{
						type: "paragraph",
						content: [
							{
								type: "text",
								content: "These addends represent thousands, hundreds, tens, and a decimal part."
							}
						]
					}
				]
	...
	const buildTensAsOnesFeedback = (): FeedbackContent<TemplateWidgets> => ({
		preamble: {
			...
			summary: [
				...
				{ type: "text", content: ". The decimal portion was named with the correct place value." }
			]
		},
		steps: [
			{
	...
	const buildWrongUnitOnlyFeedback = (): FeedbackContent<TemplateWidgets> => ({
		preamble: {
			...
			summary: [
				{
					type: "text",
					content: "The decimal was named with the wrong place value for the given digits."
				},
	...
	const buildCompressedAndWrongUnitFeedback = (): FeedbackContent<TemplateWidgets> => ({
		preamble: {
			correctness: "incorrect",
			summary: [
				{
					type: "text",
					content: "The whole number was compressed into tens and ones, and the decimal place value was misnamed."
				}
			]
		},
		steps: [
			{
				type: "step",
				title: [{ type: "text", content: "Restore the correct whole-number places" }],
				content: [
					{ type: "paragraph", content: [{ type: "text", content: "Expanded form: " }, { type: "math", mathml: expandedMathML }] },
					{
						type: "paragraph",
						content: [
							{
			...
			{
				type: "step",
				title: [{ type: "text", content: "Fix the decimal unit from its digit count" }],
				content: [
					{
						type: "paragraph",
						content: [
	...
	const feedback: Record<ChoiceId, { content: FeedbackContent<TemplateWidgets> }> = {
	...
	const assessmentItem = {
		...
		body: [
			...
			{ type: "paragraph", content: [{ type: "math", mathml: expandedMathML }] },
		...
		interactions: {
			choice_interaction: {
				...
				responseIdentifier: "RESPONSE",
				prompt: [{ type: "text", content: question }, { type: "text", content: "Select one answer." }],
